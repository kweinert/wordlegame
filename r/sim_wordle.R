...
#' @export
sim_wordle <- function(knowledge, verbose=TRUE) {
	chosen <- sample(rownames(knowledge$wl_num),1)
	i <- 1
	repeat {
		guess <- suggest_guess(knowledge=knowledge, num_guess=i, n=1, verbose=verbose)
		if(is.na(guess)) {
		...
		knowledge <- learn(knowledge=knowledge, guess=guess, reply=feedb)
		i <- i + 1
		if(i>6) break
	}
	if(!success) {
